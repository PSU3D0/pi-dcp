...
test("applyOutputBodyReplace prunes large outputs outside turn protection", () => {
  const messages: AgentMessage[] = [
    ...
    { role: "assistant", content: [{ type: "toolCall", id: "call_1", name: "read", arguments: { path: "a.txt" } }], api: "", provider: "", model: "", usage: {} as any, stopReason: "stop", timestamp: 2 },
    { role: "toolResult", toolCallId: "call_1", toolName: "read", content: [{ type: "text", text: "This is a massive file content!" }], isError: false, timestamp: 3 }, // age 2 (will age out)
  ...
  expect(messages[2].content[0].text).toContain("[DCP: Large output from read({\"path\":\"a.txt\"}...) pruned due to age (Turn 2)");
...
test("applyOutputBodyReplace keeps small outputs", () => {
  const messages: AgentMessage[] = [
    { role: "user", content: "t1", timestamp: 1 } as any, // age 2
    { role: "assistant", content: [{ type: "toolCall", id: "call_1", name: "read", arguments: { path: "a.txt" } }], api: "", provider: "", model: "", usage: {} as any, stopReason: "stop", timestamp: 2 },
    { role: "toolResult", toolCallId: "call_1", toolName: "read", content: [{ type: "text", text: "small" }], isError: false, timestamp: 3 }, // age 2 (aged out, but small)
  ...
  expect(messages[2].content[0].text).toBe("small");
...
test("applyOutputBodyReplace keeps recent large outputs", () => {
  const messages: AgentMessage[] = [
    ...
    { role: "assistant", content: [{ type: "toolCall", id: "call_1", name: "read", arguments: { path: "a.txt" } }], api: "", provider: "", model: "", usage: {} as any, stopReason: "stop", timestamp: 2 },
    { role: "toolResult", toolCallId: "call_1", toolName: "read", content: [{ type: "text", text: "This is a massive file content!" }], isError: false, timestamp: 3 }, // age 0 (recent)
  ];
  ...
  expect(messages[2].content[0].text).toBe("This is a massive file content!");
  expect(state.stats.prunedItemsCount.outputBodyReplace).toBe(0);
});
