...
test("applyPurgeErrors minimizes old errors", () => {
  const messages: AgentMessage[] = [
    ...
    { role: "toolResult", toolCallId: "call_1", toolName: "read", content: [{ type: "text", text: "Error: file not found\nline2\nline3" }], isError: true, timestamp: 3 }, // age 3
  ...
  expect(messages[1].content[0].text).toBe("[DCP: Stale error payload minimized.]\nError: file not found...");
...
test("applyPurgeErrors preserves recent errors", () => {
  const messages: AgentMessage[] = [
    ...
    { role: "toolResult", toolCallId: "call_1", toolName: "read", content: [{ type: "text", text: "Error: recent" }], isError: true, timestamp: 3 }, // age 2
  ...
  expect(messages[1].content[0].text).toBe("Error: recent");
