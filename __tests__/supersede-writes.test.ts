...
test("applySupersedeWrites prunes write arguments if superseded by later read", () => {
  const messages: AgentMessage[] = [
    {
      role: "assistant",
      content: [{ type: "toolCall", id: "write_1", name: "write", arguments: { path: "a.txt", content: "huge string" } }],
      api: "", provider: "", model: "", usage: {} as any, stopReason: "stop", timestamp: 1
    },
    { role: "toolResult", toolCallId: "write_1", toolName: "write", content: [], isError: false, timestamp: 2 },
    
    // Some time passes...
    
    {
      role: "assistant",
      content: [{ type: "toolCall", id: "read_1", name: "read", arguments: { path: "a.txt" } }],
      api: "", provider: "", model: "", usage: {} as any, stopReason: "stop", timestamp: 3
    },
    { role: "toolResult", toolCallId: "read_1", toolName: "read", content: [], isError: false, timestamp: 4 },
  ];
  ...
  expect(messages[0].content[0].arguments.content).toBe("[DCP: Content superseded by later read]");
...
test("applySupersedeWrites does NOT prune writes if no later read exists", () => {
  const messages: AgentMessage[] = [
    {
      role: "assistant",
      content: [{ type: "toolCall", id: "write_1", name: "write", arguments: { path: "b.txt", content: "huge string" } }],
      api: "", provider: "", model: "", usage: {} as any, stopReason: "stop", timestamp: 1
    },
    { role: "toolResult", toolCallId: "write_1", toolName: "write", content: [], isError: false, timestamp: 2 },
    ...
    {
      role: "assistant",
      content: [{ type: "toolCall", id: "read_1", name: "read", arguments: { path: "c.txt" } }],
      api: "", provider: "", model: "", usage: {} as any, stopReason: "stop", timestamp: 3
    },
    { role: "toolResult", toolCallId: "read_1", toolName: "read", content: [], isError: false, timestamp: 4 },
  ];
  ...
  expect(messages[0].content[0].arguments.content).toBe("huge string");
